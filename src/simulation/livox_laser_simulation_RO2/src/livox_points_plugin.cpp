...
namespace gazebo
{
    ...
    void convertDataToRotateInfo(const std::vector<std::vector<double>> &datas, std::vector<AviaRotateInfo> &avia_infos)
    {
        ...
        for (auto &data : datas)
        {
            if (data.size() == 3)
            {
                ...
                avia_infos.back().zenith = data[2] * deg_2_rad - M_PI_2;  // Convert to standard right-handed zenith.
            } else {
    ...
    void LivoxPointsPlugin::OnNewLaserScans() {
        ...
        // Iterate ray hit pairs.
        for (const auto &pair : points_pair) {
            auto range = rayShape->GetRange(pair.first);
            auto intensity = rayShape->GetRetro(pair.first);

            // Clamp out-of-range returns.
            if (range <= RangeMin() || range >= RangeMax()) {
                range = 0;
            }

            // Compute Cartesian hit.
